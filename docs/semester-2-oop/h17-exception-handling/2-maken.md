...
```
:::info
Je zou hier ook een `ArgumentException` kunnen gebruiken. `DivideByZeroException` wordt vooral door de runtime zelf gebruikt.
:::
...
:::info
Technisch gezien kan je ook  erven van `SystemException`, `ApplicationException` of andere specifieke Exceptions, maar in de documentatie staat uitdrukkelijk dat je eigen klassen best afleidt van `Exception`.
:::